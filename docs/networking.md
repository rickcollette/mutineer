...
- Telnet is default. For TLS/SSH place a terminator in front: stunnel, HAProxy `mode tcp`, or sshd `ForceCommand`. Point terminator to `mutineer` port (default 2323).
- Recommend: stunnel service `accept = 992`, `connect = 127.0.0.1:2323`, `protocol = proxy` for logging client IPs.
