...
#define DB_BIND_BLOB_VAL(ptr, n)                                               \
  ((DbBind){DB_BIND_BLOB, {.blob = {(ptr), (n)}}})
...
typedef struct DbDoor {
  ...
  int lb_enable;      /* LB_ENABLE: publish scores when non-zero */
  char lb_key[64];    /* stable public game key; defaults to door id */
  char lb_label[32];  /* score unit shown to users */
  char lb_order[8];   /* desc (higher wins) or asc (lower wins) */
} DbDoor;

typedef struct DbDoorScore {
  char game_key[64];
  char game_name[64];
  char score_label[32];
  char score_order[8];
...
int db_exec_simple(BbsDb *db,
                   const char *sql); /* returns rows affected, -1 on error */
...
int db_user_pw_age_days(
    BbsDb *db,
    int user_id); /* Returns days since password change, -1 on error */
...
int db_subscription_check_expired(
    BbsDb *db); /* Returns count of expired subscriptions processed */
...
/* Path helper to resolve within a file area safely. Returns false if traversal
 * detected. */
...
bool db_protocol_update(BbsDb *db, int id, const char *name,
                        const char *direction, const char *command, int active);
...
int db_user_scan_area_get(BbsDb *db, int user_id,
                          int area_id); /* 1=scan, 0=skip */
