...
static void console_clients_remove(ConsoleClient *cc)
{
  pthread_mutex_lock(&g_console_clients_mu);
  for (int i = 0; i < 64; i++)
    if (g_console_clients[i] == cc) g_console_clients[i] = NULL;
  pthread_mutex_unlock(&g_console_clients_mu);
}
...
static void send_error(ConsoleClient *cc, const char *id, const char *err)
{
  ...
  snprintf(out, sizeof(out), "{\"id\":%s,\"ok\":false,\"error\":%s}", idj, errj);
  send_json(cc, out);
}
...
static int read_line_timeout(int fd, char *buf, size_t cap, int timeout_sec)
{
  ...
  while (o + 1 < cap)
  {
    ...
    FD_ZERO(&rfds);
    FD_SET(fd, &rfds);
    struct timeval tv = {.tv_sec = timeout_sec > 0 ? timeout_sec : 600, .tv_usec = 0};
...
static bool json_looks_valid(const char *line)
{
  ...
  for (const char *p = line; *p; p++)
  {
    ...
    else if (c == ':') saw_colon = true;
    if (depth < 0) return false;
  }
  return depth == 0 && !in_str && saw_colon && line[0] == '{';
}

static bool valid_node_num(int node)
{
  return node > 0 && node <= 256;
}

static bool require_valid_node(ConsoleClient *cc, const char *id, int node)
{
  if (valid_node_num(node)) return true;
...
{
  struct statvfs st;
  if (statvfs(path ? path : ".", &st) != 0) return 0;
...
static void payload_stats(ConsoleClient *cc, char *out, size_t cap)
{
  ...
  snprintf(out, cap,
           ...
           "\"minutes_today\":%d,\"errors\":%d,\"total_calls\":%d,\"total_posts\":%d,"
           ...
           daily.uploads, daily.downloads, (long long)daily.ul_kb, (long long)daily.dl_kb,
           daily.minutes, daily.errors, totals.total_calls, totals.total_posts,
           totals.total_uploads, totals.total_downloads, totals.days_online,
           totals.total_users, sysop.smw, (long long)disk_free_kb(cc->cfg.data_path), os);
}
...
static void payload_logs(ConsoleClient *cc, char *out, size_t cap)
{
  ...
  if (f)
  {
    while (fgets(ring[count % 40], sizeof(ring[0]), f)) count++;
    fclose(f);
  }
  size_t o = 0;
...
static void payload_inspect(ConsoleClient *cc, int node, char *out, size_t cap)
{
  DbNode nodes[64];
  int n = db_node_list(cc->db, nodes, 64);
  DbNode *found = NULL;
  for (int i = 0; i < n; i++)
    if (nodes[i].node_num == node) found = &nodes[i];

  size_t o = 0;
  o += (size_t)snprintf(out + o, cap - o, "\"node\":{\"node\":%d,", node);
  if (!found)
  {
    o += (size_t)snprintf(out + o, cap - o, "\"status\":\"inactive\",\"handle\":\"\",\"ip\":\"\",\"activity\":\"\"");
  }
  else
  {
    ...
    o += (size_t)snprintf(out + o, cap - o, "\"status\":%s,\"handle\":%s,\"ip\":%s,\"activity\":%s",
                          st, h, ip, act);
  }
  ...
  snprintf(out + o, cap - o, ",\"locked\":%s}", online_node_is_locked(node) ? "true" : "false");
}

static bool authenticate_console(ConsoleClient *cc, const char *user, const char *password)
{
  DbUser u;
  memset(&u, 0, sizeof(u));
...
static void passthrough_action(ConsoleClient *cc, const char *action)
{
  ...
  s.time_left_min = cc->user.time_limit_min > 0 ? cc->user.time_limit_min : cc->cfg.session_time_limit_min;
...
static void run_shell(ConsoleClient *cc, const char *id)
{
  ...
  char errbuf[256] = {0};
  char **argv = NULL;
  if (!bbs_argv_parse_template(cc->cfg.wfc_shell_command, NULL, &argv, errbuf, sizeof(errbuf)))
  {
  ...
  bbs_exec_argv(argv, "console-shell", NULL, cc->fd, cc->fd, cc->fd, 0, &result, errbuf, sizeof(errbuf));
  bbs_argv_free(argv);
  send_json(cc, "{\"event\":\"passthrough.end\"}");
...
static void handle_command(ConsoleClient *cc, const char *line)
{
  ...
  if (!cmd[0]) json_get_string(line, "type", cmd, sizeof(cmd));
  ...
  else if (!strcmp(cmd, "node.inspect"))
  {
    ...
    if (!require_valid_node(cc, id, node)) return;
  ...
  else if (!strcmp(cmd, "node.kick"))
  {
    int node = json_get_int(line, "node", 0);
    if (!require_valid_node(cc, id, node)) return;
    bool ok = online_mark_node_dead(node, NULL, "\r\nYou have been disconnected by the sysop.\r\n");
    if (ok) db_node_upsert(cc->db, node, 0, "kicked", "console", cc->ip);
    if (ok) emit_node_event("kick", node);
  ...
  else if (!strcmp(cmd, "node.lock") || !strcmp(cmd, "node.unlock"))
  {
    ...
    db_node_upsert(cc->db, node, 0, lock ? "locked" : "idle", "console", cc->ip);
    log_audit(cc->user.handle, lock ? "console_node_lock" : "console_node_unlock", cc->ip);
  ...
  else if (!strcmp(cmd, "menu.session.start"))
  {
    ...
    if (!action[0]) send_error(cc, id, "missing action");
    else
    {
...
static void *console_client_thread(void *arg)
{
  ...
  for (;;)
  {
    int n = read_line_timeout(cc->fd, line, sizeof(line), cc->cfg.console_idle_timeout_sec);
    if (n <= 0) break;
    handle_command(cc, line);
  }
  if (cc->db) db_close(cc->db);
...
{
  (void)db;
  if (!cfg || !cfg->console_enabled) return;
  ...
  {
    log_warn("console service binding to non-loopback address %s", g_console.cfg.console_bind);
  }
  g_console.listen_fd = make_listener(g_console.cfg.console_bind, g_console.cfg.console_port);
  if (g_console.listen_fd < 0)
  {
    log_error("console service bind failed on %s:%d", g_console.cfg.console_bind, g_console.cfg.console_port);
    return;
  }
  g_console.running = 1;
  if (pthread_create(&g_console.thread, NULL, console_service_thread, &g_console) != 0)
  {
  ...
  log_info("console service listening on %s:%d", g_console.cfg.console_bind, g_console.cfg.console_port);
}
